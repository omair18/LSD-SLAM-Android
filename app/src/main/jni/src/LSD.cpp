...
void run(SlamSystem * system, Undistorter* undistorter, Output3DWrapper* outputWrapper, Sophus::Matrix3f K)
{
    ...
    for(unsigned int i = 0; i < numFrames; i++)
    {
        ...
        if(runningIDX == 0)
        {
            ...
            printf("system->randomInit\n");
            system->randomInit(image.data, fakeTimeStamp, runningIDX);
            //LOGD("system->randomInit done.");
            printf("system->randomInit done.");
        }
        else
        {
            //LOGD("system->trackFrame");
            printf("system->trackFrame");
            system->trackFrame(image.data, runningIDX, hz == 0, fakeTimeStamp);
            printf("system->trackFrame done.");
...
#define CALIB_FILE "/home/omair/workspace/lsd_slam-master/lsd_slam_core/jni/src/LSD_room/cameraCalibration.cfg"
#define IMAGE_DIR "/home/omair/workspace/lsd_slam-master/lsd_slam_core/jni/src/LSD_room/images"
